...
function applySuperSpans (selector) {
  ...
  if (element) {
    ...
    console.warn(
      `[addSuperScript] Element with selector "${selector}" not found.`
    )
  }
}
document.addEventListener('DOMContentLoaded', () => {
...
function applySubSpans (selector) {
  ...
  if (element) {
    ...
    console.warn(
      `[addSubScript] Element with selector "${selector}" not found.`
    )
  }
}
document.addEventListener('DOMContentLoaded', () => {
...
// addSVG.js
// -----------------------------------------------------------------------------
function addSVG (text) {
  const svGex = /\[icon-(.*?)\]/g
  const svgReplace = (match, content) => {
    return `<span class="icon icon-${content}"></span>`
  }
  return text.replace(svGex, svgReplace)
}
function applySVGSpans (selector) {
  const element = document.querySelector(selector)
  if (element) {
    // CAUTION: Modifying innerHTML can destroy event listeners on child elements.
    // Best used on elements with static content.
    element.innerHTML = addSVG(element.innerHTML)
  } else {
    console.warn(`[addSVG] Element with selector "${selector}" not found.`)
  }
}
document.addEventListener('DOMContentLoaded', () => {
  applySVGSpans('body')
