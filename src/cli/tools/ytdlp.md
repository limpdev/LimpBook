...
Options:
  General Options:
    ...
    --color [STREAM:]POLICY                           Whether to emit color codes in output, optionally prefixed by the
                                                      ...
                                                      terminal sequences). Can be used multiple times
  ...
  Video Selection:
    ...
    --break-per-input                                 Alters --max-downloads, --break-on-existing, --break-match-filter,
                                                      and autonumber to reset per input URL
  ...
  Verbosity and Simulation Options:
    ...
    --print-to-file [WHEN:]TEMPLATE FILE              Append given template to the file. The values of WHEN and TEMPLATE
                                                      are same as that of --print. FILE uses the same syntax as the
  ...
  Post-Processing Options:
    ...
    --concat-playlist POLICY                          Concatenate videos in a playlist. One of "never", "always", or
                                                      ...
                                                      All the video files must have same codecs and number of streams to
                                                      be concatable. The "pl_video:" prefix can be used with "--paths"
    ...
    --fixup POLICY                                    Automatically correct known faults of the file. One of never (do
                                                      ...
                                                      fix file if we can, warn otherwise), force (try fixing even if
                                                      file already exists)
    ...
    --exec [WHEN:]CMD                                 Execute a command, optionally prefixed with when to execute it,
                                                      ...
                                                      the command. If no fields are passed, %(filepath,_filename|)q is
                                                      appended to the end of the command. This option can be used
                                                      multiple times
    --no-exec                                         Remove any previously defined --exec
    --convert-subs FORMAT                             Convert the subtitles to another format (currently supported: ass,
                                                      lrc, srt, vtt) (Alias: --convert-subtitles)
  ...
  Extractor Options:
    ...
    --hls-split-discontinuity                         Split HLS playlists to different formats at discontinuities such
                                                      as ad breaks
    --no-hls-split-discontinuity                      Do not split HLS playlists to different formats at discontinuities
